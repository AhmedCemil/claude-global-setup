...
a shared browser driver, and a two-way drawing canvas.
...
- **No-install where possible.** `context_usage.ps1` reads the transcript directly — no spawning,
  no side effects. The limit script needs Python; nothing else does.
