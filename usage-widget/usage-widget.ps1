...
function Get-Usage {
    ...
    try {
        # -s silent, -m timeout, -w append HTTP status so we can detect 403 without -f swallowing the body
        $out = & $curl -s -m $ApiTimeout $url -H "Cookie: $cookie" -A 'claude-usage-widget/1.0' -w "`n%{http_code}" 2>$null
        $lines = $out -split "`n"
        $status = $lines[-1].Trim()
        $body = ($lines[0..($lines.Count - 2)] -join "`n")
        # 403 = rate-limited by Cloudflare; 401 = bad/expired session.
        # Flag rateLimited so the UI can keep showing the last good value with "(RL)".
        ...
        # reset values are returned as RAW ISO strings so the time-format toggle can
        # re-render instantly without a refetch.
        $result = @{ ok = $true; pct = [double]$five.utilization; resetIso = $five.resets_at
                     hasWeekly = $false; wpct = $null; wresetIso = '' }
        # 7-day (weekly) limit — always parsed; the "Show 7d" toggle controls display.
        # Some accounts return null here (no weekly limit); hasWeekly = $false → UI shows "7d: n/a".
...
$lblWeekly.Font = New-Object System.Drawing.Font('Segoe UI', 9, [System.Drawing.FontStyle]::Bold)
...
$lblReset.Font = New-Object System.Drawing.Font('Consolas', 8, [System.Drawing.FontStyle]::Regular)
...
$form.Add_Shown({ & $doRefresh; $timer.Start() })
$form.Add_FormClosed({
